...
3. Elegir el marco de pruebas (el ejemplo se ha realizado con MSTestv2 [@video_mstest_2019]) y confirmar.

Una vez creada la clase de pruebas, se definen métodos decorados con atributos como `[TestMethod]` (**Código 8.1**), y se utilizan mecanismos como *mocks* o *stubs* si el componente tiene dependencias externas.
...
Además, WinUI 3 es compatible con tecnologías asistidas como el Narrador de Windows [@doc_narrador]. Para ello, se propone evolucionar la interfaz con propiedades de automatización (Microsoft UI Automation [@doc_ui_automation_2025]) en los controles de la interfaz, que permiten describir su propósito. Esto permitiría que los usuarios que utilizan esta función de Windows reciban descripciones precisas de las funciones disponibles en pantalla y acceder a las mismas.
...
```
![Código 8.2. Ejemplo de un botón de guardar con propiedades de automatización.](./Pictures/remove.png)

### 8.3.4. Soporte a nuevo proveedor de IA: Mistral

**Mistral** [@company_mistral_ai] es un proveedor europeo de modelos de lenguaje generativo que ofrece tanto opciones gratuitas como comerciales mediante una API *OpenAI-compatible* [@doc_mistral_api]. Este último aspecto es especialmente relevante, ya que permite reutilizar buena parte de la lógica ya implementada en `OpenAIService`, minimizando el esfuerzo necesario para su integración. La inclusión de Mistral no solo ampliaría la oferta de modelos disponibles, sino que fortalecería la independencia tecnológica frente a los grandes proveedores actuales.
...
- **Documentos, búsqueda web e imágenes**: Dotar a las conversaciones y agentes de herramientas como la búsqueda de conocimiento en documentos y la búsqueda web, añadiendo al contexto de la conversación el contenido encontrado con los términos introducidos por el usuario, enriqueciendo así las respuestas generadas. Igualmente permitir el reconocimiento de imágenes con modelos *open source* como Llama 3.2 Vision [@doc_llama32vision_2025].
